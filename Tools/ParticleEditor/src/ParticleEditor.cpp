...
void ParticleEditor::Update()
{
	...
	if (ImGui::BeginMainMenuBar()) {
		if (ImGui::BeginMenu(LANGS("文件", "File"))) {
			if (ImGui::Button(LANGS("导出代码到剪贴板", "Export Code to Clipboard"))) {
		...
		if (ImGui::BeginMenu(LANGS("选项", "Options"))) {
			...
			if (ImGui::BeginMenu(LANGS("语言", "Language"))) {
				ImGui::Combo(LANGS("语言", "Language"), &lang_i, "中文\0English\0");
				ImGui::EndMenu();
			}
			ImGui::EndMenu();
		}
	...
	ImGui::Begin("Particle Editor", nullptr, windowFlags);
	{
		...
		ImGui::Indent();
		{
			...
			if (ImGui::SliderFloat(LANGS("发射间隔", "Interval"), &s_interval, 1.f / 60.f, 5.f)) {
				s_particleEmitter->Interval(s_interval);
			}
			if (ImGui::DragInt(LANGS("发射数量", "Amount"), &s_amount, 1, 1, 1000)) {
				s_particleEmitter->Amount(s_amount);
			}
			if (ImGui::DragFloat2(LANGS("位置范围", "Position Range"), &s_positionRange.x, 1.0f, 0.f, 1000.f)) {
		...
		ImGui::Text(LANGS("速度:", "Speed:"));
		ImGui::Indent();
		{
			ImGui::SliderFloat(LANGS("最小速度", "Speed Min"), &s_particleType->m_speedMin, 0.f, 500.f);
			ImGui::SliderFloat(LANGS("最大速度", "Speed Max"), &s_particleType->m_speedMax, 0.f, 500.f);
			ImGui::SliderFloat(LANGS("乘数", "Multiplier"), &s_particleType->m_speedMultiplier, 0.f, 10.f);
			ImGui::DragFloat2(LANGS("加速度", "Acceleration"), &s_particleType->m_acceleration.x, 1.f, -500.f, 500.f);
			ImGui::SliderFloat(LANGS("摩擦力", "Friction"), &s_particleType->m_friction, 0.f, 500.f);
		}
		ImGui::Unindent();
		ImGui::Separator();

		ImGui::Text(LANGS("方向:", "Direction:"));
		ImGui::Indent();
		{
		...
		ImGui::Text(LANGS("生命周期:", "Life:"));
		ImGui::Indent();
		{
			ImGui::SliderFloat(LANGS("最小生命周期", "Life Min"), &s_particleType->m_lifeMin, 0.f, 10.f);
			ImGui::SliderFloat(LANGS("最大生命周期", "Life Max"), &s_particleType->m_lifeMax, 0.f, 10.f);
		}
		ImGui::Unindent();
		ImGui::Separator();

		ImGui::Text(LANGS("大小:", "Size:"));
		ImGui::Indent();
		{
			ImGui::DragFloat(LANGS("大小", "Size"), &s_particleType->m_size, 0.1f, 0.f, 10.f);
			ImGui::DragFloat(LANGS("大小范围", "Range##Size"), &s_particleType->m_sizeRange, 0.1f, 0.f, 10.f);
		}
		ImGui::Unindent();
		ImGui::Separator();

		ImGui::Text(LANGS("转向:", "Rotation:"));
		ImGui::Indent();
		{
			ImGui::SliderFloat(LANGS("最小旋转速度", "Spin Min"), &s_particleType->m_spinMin, 0.f, Math::Pi2 * 5.f);
		...
		ImGui::Text(LANGS("其他:", "Other:"));
		ImGui::Indent();
		{
			ImGui::Checkbox(LANGS("缩放渐隐", "Scale Out"), &s_particleType->m_scaleOut);
			ImGui::Checkbox(LANGS("使用实际增量时间", "Use Actual Delta Time"), &s_particleType->m_useActualDeltaTime);
		}
		ImGui::Unindent();
		ImGui::EndChild();
	}
