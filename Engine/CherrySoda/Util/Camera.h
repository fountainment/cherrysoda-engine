...
public:
	...
	CHERRYSODA_GETTER_SETTER_EX_OF_VEC3(Position, m_position, CHERRYSODA_NONE_OP, if (m_position == v) return;
										m_changed = true);
	CHERRYSODA_GETTER_SETTER_EX_OF_VEC3(Scale, m_zoom, CHERRYSODA_NONE_OP, if (m_zoom == v) return; m_changed = true);
	CHERRYSODA_GETTER_SETTER_EX_OF_VEC3(Origin, m_origin, CHERRYSODA_NONE_OP, if (m_origin == v) return;
										m_changed = true);
	CHERRYSODA_GETTER_SETTER_EX_OF_VEC3(Direction, m_direction, CHERRYSODA_NONE_OP, if (m_direction == v) return;
										m_changed = true);

	CHERRYSODA_GETTER_SETTER_EX_OF_TYPE(float, ZRotation, m_zAngle, CHERRYSODA_NONE_OP, if (m_zAngle == v) return;
										m_changed = true);
	CHERRYSODA_GETTER_SETTER_EX_OF_TYPE(float, Width, m_width, CHERRYSODA_NONE_OP, if (m_width == v) return;
										Ratio(v / Height()); m_changed = true);
	CHERRYSODA_GETTER_SETTER_EX_OF_TYPE(float, Height, m_height, CHERRYSODA_NONE_OP, if (m_height == v) return;
										Ratio(Width() / v); m_changed = true);
	CHERRYSODA_GETTER_SETTER_EX_OF_TYPE(float, FOV, m_fov, CHERRYSODA_NONE_OP, if (m_fov == v) return;
										m_changed = true);

	CHERRYSODA_GETTER_SETTER_EX_OF_TYPE(float, Ratio, m_ratio, CHERRYSODA_NONE_OP, if (m_ratio == v) return;
										m_changed = true);
	CHERRYSODA_GETTER_SETTER_EX_OF_BOOL(UseOrthoProjection, m_orthoProjection, CHERRYSODA_NONE_OP,
	...
	inline void SetUpVector(const Math::Vec3& vec)
	{
		if (m_upVector != vec) {
			m_upVector = vec;
			m_changed = true;
	...
	inline const Math::Mat4* GetViewMatrix()
	{
		if (m_changed) {
			UpdateMatrices();
		}
	...
	inline const Math::Mat4* GetProjectionMatrix()
	{
		if (m_changed) {
			UpdateMatrices();
		}
	...
	inline float Left()
	{
		...
		return (m_inverseMatrix * Math::Vec4(-Vec3_XUp, 1.f)).x;
	}
	inline float Right()
	{
		if (m_changed) {
	...
	inline float Bottom()
	{
		...
		return (m_inverseMatrix * Math::Vec4(-Vec3_YUp, 1.f)).y;
	}
	inline float Top()
	{
		if (m_changed) {
