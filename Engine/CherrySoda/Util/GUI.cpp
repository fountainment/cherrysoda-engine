...
void GUI::Initialize()
{
	...
	io.ConfigFlags |=
		ImGuiConfigFlags_DockingEnable | ImGuiConfigFlags_NavEnableGamepad | ImGuiConfigFlags_NavEnableKeyboard;
	...
	if (Engine::Instance()->IsActive()) {
		...
		if ((io.ConfigFlags & ImGuiConfigFlags_NavEnableGamepad) != 0) {
			...
			if (gamepad->Attached()) {
				...
				io.AddKeyAnalogEvent(ImGuiKey_GamepadLStickLeft, LS.x < -stickDeadZone,
									 Math_Clamp(LS.x, -1.f, 0.f) * -1.f);
				...
				io.AddKeyAnalogEvent(ImGuiKey_GamepadLStickDown, LS.y < -stickDeadZone,
									 Math_Clamp(LS.y, -1.f, 0.f) * -1.f);
				io.AddKeyAnalogEvent(ImGuiKey_GamepadRStickLeft, RS.x < -stickDeadZone,
									 Math_Clamp(RS.x, -1.f, 0.f) * -1.f);
				...
				io.AddKeyAnalogEvent(ImGuiKey_GamepadRStickDown, RS.y < -stickDeadZone,
									 Math_Clamp(RS.y, -1.f, 0.f) * -1.f);
			}
			else {
				io.BackendFlags &= ~ImGuiBackendFlags_HasGamepad;
			}
...
void GUI::UpdateConsole()
{
	...
	if (Engine::Instance()->ConsoleOpened()) {
		...
		ImGui::Begin("Console");
		{
			...
			bool commandInput =
				ImGui::InputText("##Command", Commands::ms_currentText, IM_ARRAYSIZE(Commands::ms_currentText),
								 inputTextFlag, Funcs::InputTextCallback);
			ImGui::PopItemWidth();
			ImGui::SameLine();
			commandInput |= ImGui::Button("Enter");
...
void GUI::Render()
{
	...
	Graphics::UseCurrentRenderPass()->SetViewProjectionMatrix(
		Math_Identity<Math::Mat4>(),
		Math_Ortho(Graphics::TexelHalf(), io.DisplaySize.x + Graphics::TexelHalf(),
				   io.DisplaySize.y + Graphics::TexelHalf(), Graphics::TexelHalf(), -1000.f, 1000.f));
	for (int n = 0; n < drawData->CmdListsCount; ++n) {
		const ImDrawList* drawList = drawData->CmdLists[n];
		auto vb = Graphics::CreateTransientVertexBuffer(
			reinterpret_cast<Graphics::ImGuiVertex*>(drawList->VtxBuffer.Data), drawList->VtxBuffer.Size);
