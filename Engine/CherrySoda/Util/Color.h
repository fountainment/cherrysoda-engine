...
public:
	...
	constexpr Color(float r, float g, float b, float a) : m_r(r), m_g(g), m_b(b), m_a(a) {}
	...
	constexpr Color(int r, int g, int b, int a) : Color(r / 255.f, g / 255.f, b / 255.f, a / 255.f) {}

	constexpr Color(const char* str) : Color(0.0f)
	{
		int len = 0;
		while (str[len] != '\0')
			len++;
		...
		switch (len) {
		case 4:
			r = StringUtil::HexStrToUInt32(str + 1, 1);
			r = (r << 4) | r;
			g = StringUtil::HexStrToUInt32(str + 2, 1);
			g = (g << 4) | g;
			b = StringUtil::HexStrToUInt32(str + 3, 1);
			b = (b << 4) | b;
			break;
		case 5:
			r = StringUtil::HexStrToUInt32(str + 1, 1);
			...
			b = StringUtil::HexStrToUInt32(str + 3, 1);
			b = (b << 4) | b;
			a = StringUtil::HexStrToUInt32(str + 4, 1);
			a = (a << 4) | a;
			break;
		case 7:
	...
	{
		return F2U32(A()) << 24 | F2U32(B()) << 16 | F2U32(G()) << 8 | F2U32(R());
	}
