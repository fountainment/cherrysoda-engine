...
void Camera::UpdateMatrices()
{
	if (UseOrthoProjection()) {
		...
		m_viewMatrix = Math_Translate(
			Math_Scale(Math_Rotate(Math_Translate(Math_Identity<Math::Mat4>(), Origin()), -ZRotation(), Vec3_ZUp),
					   Scale()),
			-Position());
		m_projMatrix = Math_Ortho(texelHalfVec.x, m_width + texelHalfVec.x, -texelHalfVec.y, m_height - texelHalfVec.y,
								  -10000.f, 10000.f);
		...
		m_viewMatrix =
			Math_LookAt(actualPos, actualPos + Direction(), Math_RotateVector(GetUpVector(), ZRotation(), Vec3_ZUp));
