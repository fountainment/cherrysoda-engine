...
void Draw::LineAngle(const Math::Vec2& start, float angle, float length, const Color& color /* = Color::White*/,
					 float thickness /* = 1.f*/)
{
	GetSpriteBatch()->Draw(PixelTexture().Texture(), start, PixelTexture().ClipRect(), color, angle, IVec2_Zero,
						   Math::Vec2(length, thickness), SpriteEffects::None, 0);
}

void Draw::Line(const Math::Vec2& start, const Math::Vec2& end, const Color& color /* = Color::White*/,
				float thickness /* = 1.f*/)
{
...
void Draw::Circle(const Math::Vec2& position, float radius, const Color& color /* = Color::White*/,
