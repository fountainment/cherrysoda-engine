...
void RendererBase::RenderPrepare(Scene* /*scene*/)
{
	...
	// the screen render pass covers the drawable pixels while the camera keeps
	// the logical view size, scaling content up on high-DPI displays
	Math::IVec2 viewportSize = GetRenderTarget() != nullptr ? renderTargetSize : Engine::Instance()->GetPixelSize();
