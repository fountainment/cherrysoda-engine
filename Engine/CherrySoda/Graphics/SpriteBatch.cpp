...
void SpriteBatch::Draw(const Texture2D& tex, const Math::Vec2& pos, const Math::IRectangle& rect, const Color& color,
					   ...
					   float layerDepth)
{
	...
	const Math::Vec4 rPos = ((effects & SpriteEffects::RoundRenderingPosition) == SpriteEffects::RoundRenderingPosition)
								? Math::Vec4(Math_Round(pos), 0.f, 0.f)
								: Math::Vec4(pos, 0.f, 0.f);
	...
	const Math::Mat4 rotationMat =
		rotation == 0 ? Math_Identity<Math::Mat4>() : Math_Rotate(Math_Identity<Math::Mat4>(), rotation, Vec3_ZUp);
	AddQuad(MK_VERT(Math::Vec3(rPos + rotationMat * Math::Vec4(RBCorner, -layerDepth, 1.f)), color,
					Math::Vec2(rightUV, bottomUV)),
			MK_VERT(Math::Vec3(rPos + rotationMat * Math::Vec4(RTCorner, -layerDepth, 1.f)), color,
					Math::Vec2(rightUV, topUV)),
			MK_VERT(Math::Vec3(rPos + rotationMat * Math::Vec4(LBCorner, -layerDepth, 1.f)), color,
					Math::Vec2(leftUV, bottomUV)),
			MK_VERT(Math::Vec3(rPos + rotationMat * Math::Vec4(LTCorner, -layerDepth, 1.f)), color,
					Math::Vec2(leftUV, topUV)));
