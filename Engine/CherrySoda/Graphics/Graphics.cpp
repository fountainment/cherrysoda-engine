...
static uint64_t s_defaultTextureSamplerFlags = 0 | BGFX_SAMPLER_U_CLAMP | BGFX_SAMPLER_V_CLAMP;

static constexpr uint64_t s_linearTextureSamplerFlags = 0 | BGFX_SAMPLER_U_CLAMP | BGFX_SAMPLER_V_CLAMP;

static constexpr uint64_t s_pointTextureSamplerFlags = 0 | BGFX_SAMPLER_MIN_POINT | BGFX_SAMPLER_MAG_POINT |
...
{
	PosColorDefinition::s_layout.begin()
...
{
	PosColorNormalDefinition::s_layout.begin()
		.add(bgfx::Attrib::Position, 3, bgfx::AttribType::Float)
		.add(bgfx::Attrib::Color0, 4, bgfx::AttribType::Uint8, true)
...
{
	PosColorTexCoord0Definition::s_layout.begin()
		.add(bgfx::Attrib::Position, 3, bgfx::AttribType::Float)
		.add(bgfx::Attrib::Color0, 4, bgfx::AttribType::Uint8, true)
...
{
	PosNormalTexCoord0Definition::s_layout.begin()
		.add(bgfx::Attrib::Position, 3, bgfx::AttribType::Float)
		.add(bgfx::Attrib::Normal, 4, bgfx::AttribType::Uint8, true, true)
		.add(bgfx::Attrib::TexCoord0, 2, bgfx::AttribType::Float)
		.end();
...
{
	PosColorNormalTexCoord0Definition::s_layout.begin()
		.add(bgfx::Attrib::Position, 3, bgfx::AttribType::Float)
		.add(bgfx::Attrib::Color0, 4, bgfx::AttribType::Uint8, true)
		.add(bgfx::Attrib::Normal, 4, bgfx::AttribType::Uint8, true, true)
		.add(bgfx::Attrib::TexCoord0, 2, bgfx::AttribType::Float)
...
{
	ImGuiVertexDefinition::s_layout.begin()
...
void Graphics::Initialize()
{
	...
	ms_blendFunctions[(int)Graphics::BlendFunction::Default] =
		BGFX_STATE_BLEND_FUNC(BGFX_STATE_BLEND_ONE, BGFX_STATE_BLEND_INV_SRC_ALPHA);
	ms_blendFunctions[(int)Graphics::BlendFunction::Alpha] =
		BGFX_STATE_BLEND_FUNC_SEPARATE(BGFX_STATE_BLEND_SRC_ALPHA, BGFX_STATE_BLEND_INV_SRC_ALPHA, BGFX_STATE_BLEND_ONE,
									   BGFX_STATE_BLEND_INV_SRC_ALPHA);
	ms_blendFunctions[(int)Graphics::BlendFunction::Add] =
		BGFX_STATE_BLEND_FUNC(BGFX_STATE_BLEND_ONE, BGFX_STATE_BLEND_ONE);
	ms_blendFunctions[(int)Graphics::BlendFunction::Max] =
		BGFX_STATE_BLEND_FUNC(BGFX_STATE_BLEND_ONE, BGFX_STATE_BLEND_ONE) |
		BGFX_STATE_BLEND_EQUATION(BGFX_STATE_BLEND_EQUATION_MAX);
	ms_blendFunctions[(int)Graphics::BlendFunction::Min] =
		BGFX_STATE_BLEND_FUNC(BGFX_STATE_BLEND_ONE, BGFX_STATE_BLEND_ONE) |
		BGFX_STATE_BLEND_EQUATION(BGFX_STATE_BLEND_EQUATION_MIN);
	ms_blendFunctions[(int)Graphics::BlendFunction::Multiply] =
...
void Graphics::Terminate()
{
	...
	if (const bgfx::Stats* stats = bgfx::getStats()) {
		...
		CHERRYSODA_DEBUG_FORMAT("bgfx live handles before shutdown (includes bgfx-internal resources): textures=%d "
								"programs=%d shaders=%d uniforms=%d frameBuffers=%d "
								"vertexBuffers=%d indexBuffers=%d dynamicVertexBuffers=%d dynamicIndexBuffers=%d "
								"textureMemoryUsed=%lld rtMemoryUsed=%lld\n",
								stats->numTextures, stats->numPrograms, stats->numShaders, stats->numUniforms,
								stats->numFrameBuffers, stats->numVertexBuffers, stats->numIndexBuffers,
								stats->numDynamicVertexBuffers, stats->numDynamicIndexBuffers,
...
#endif
	bgfx::reset(Engine::Instance()->GetWidth(), Engine::Instance()->GetHeight(),
...
{
	bgfx::setViewClear(RenderPass(), BGFX_CLEAR_COLOR | BGFX_CLEAR_DEPTH, color.U32(), 1.0f, 0);
}
...
void Graphics::SetTransientIndexBuffer(Graphics::TransientIndexBufferHandle indexBuffer, size_t startIndex,
									   size_t indexAmount)
{
...
void Graphics::SetStateDefault(Graphics::BlendFunction blendFunc /* = Graphics::BlendFunction::Normal*/,
							   Graphics::PrimitiveType primitiveType /* = Graphics::PrimitiveType::Triangles*/)
{
	constexpr uint64_t stateDefault = BGFX_STATE_WRITE_RGB | BGFX_STATE_WRITE_A | BGFX_STATE_WRITE_Z |
									  BGFX_STATE_DEPTH_TEST_LEQUAL | BGFX_STATE_CULL_CW | BGFX_STATE_MSAA;
	bgfx::setState(stateDefault | ms_blendFunctions[(int)blendFunc] | ms_primitiveTypes[(int)primitiveType]);
}

void Graphics::SetStateNoDepth(Graphics::BlendFunction blendFunc /* = Graphics::BlendFunction::Normal*/,
							   Graphics::PrimitiveType primitiveType /* = Graphics::PrimitiveType::Triangles*/)
{
	constexpr uint64_t stateNoDepth = BGFX_STATE_WRITE_RGB | BGFX_STATE_WRITE_A | BGFX_STATE_MSAA;
	bgfx::setState(stateNoDepth | ms_blendFunctions[(int)blendFunc] | ms_primitiveTypes[(int)primitiveType]);
}
...
{
	return bgfx::createIndexBuffer(bgfx::makeRef(STL::Data(indices), static_cast<uint32_t>(STL::ByteSize(indices))))
		.idx;
}
...
Graphics::TransientIndexBufferHandle Graphics::CreateTransientIndexBuffer(const type::UInt16* indices,
																		  type::UInt32 indexAmount)
{
	if (!indexAmount) return Graphics::InvalidHandle;
	if (indexAmount == bgfx::getAvailTransientIndexBuffer(indexAmount)) {
		bgfx::TransientIndexBuffer ib{};
...
void Graphics::UpdateDynamicIndexBuffer(Graphics::DynamicIndexBufferHandle handle, int index,
										const STL::Vector<type::UInt16>& indices)
{
	bgfx::DynamicIndexBufferHandle hdl = {handle};
	bgfx::update(hdl, index, bgfx::makeRef(STL::Data(indices), static_cast<uint32_t>(STL::ByteSize(indices))));
}
...
Graphics::TextureHandle Graphics::CreateTexture2DFromRGBA(void* data, int width, int height)
{
	...
	return bgfx::createTexture2D(width, height, false, 1, bgfx::TextureFormat::Enum::RGBA8, GetTextureSamplerFlags(),
								 mem)
		.idx;
}
...
Graphics::UniformHandle Graphics::CreateUniformVec4(const String& uniform, type::UInt16 num)
{
	CHERRYSODA_ASSERT_FORMAT(!STL::ContainsKey(ms_uniformHashMap, uniform), "Uniform '%s' already exists.\n",
							 uniform.c_str());
	Graphics::UniformHandle handle =
		bgfx::createUniform(uniform.c_str(), bgfx::UniformFreq::Frame, bgfx::UniformType::Vec4, num).idx;
...
Graphics::UniformHandle Graphics::CreateUniformMat4(const String& uniform)
{
	CHERRYSODA_ASSERT_FORMAT(!STL::ContainsKey(ms_uniformHashMap, uniform), "Uniform '%s' already exists.\n",
							 uniform.c_str());
	Graphics::UniformHandle handle =
		bgfx::createUniform(uniform.c_str(), bgfx::UniformFreq::Frame, bgfx::UniformType::Mat4).idx;
	ms_uniformHashMap[uniform] = handle;
	return handle;
}
...
#define CHERRYSODA_CREATE_DYNAMIC_VERTEX_BUFFER(VERTEX_D)                                              \
	Graphics::DynamicVertexBufferHandle Graphics::CreateDynamicVertexBuffer(                           \
		const STL::Vector<VERTEX_D::VertexType>& vertices)                                             \
	{                                                                                                  \
...
#define CHERRYSODA_UPDATE_DYNAMIC_VERTEX_BUFFER(VERTEX_D)                                                             \
	void Graphics::UpdateDynamicVertexBuffer(Graphics::DynamicVertexBufferHandle handle, int index,                   \
											 const STL::Vector<VERTEX_D::VertexType>& vertices)                       \
	{                                                                                                                 \
		bgfx::DynamicVertexBufferHandle hdl = {handle};                                                               \
		bgfx::update(hdl, index, bgfx::makeRef(STL::Data(vertices), static_cast<uint32_t>(STL::ByteSize(vertices)))); \
	}

#define CHERRYSODA_CREATE_TRANSIENT_VERTEX_BUFFER(VERTEX_D)                                                           \
	Graphics::TransientVertexBufferHandle Graphics::CreateTransientVertexBuffer(                                      \
		const STL::Vector<VERTEX_D::VertexType>& vertices)                                                            \
	{                                                                                                                 \
	...
	Graphics::TransientVertexBufferHandle Graphics::CreateTransientVertexBuffer(const VERTEX_D::VertexType* vertices, \
																				type::UInt32 vertexAmount)            \
	{                                                                                                                 \
		if (!vertexAmount) return Graphics::InvalidHandle;                                                            \
		if (vertexAmount == bgfx::getAvailTransientVertexBuffer(vertexAmount, VERTEX_D::s_layout)) {                  \
			bgfx::TransientVertexBuffer vb{};                                                                         \
