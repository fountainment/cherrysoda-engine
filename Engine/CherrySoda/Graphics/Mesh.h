...
template<class VERTEX_T> class Mesh : public MeshInterface
{
public:
	...
	inline void AddCube(const VERTEX_T& v1, const VERTEX_T& v2, const VERTEX_T& v3, const VERTEX_T& v4,
	...
	{
		CHERRYSODA_ASSERT(GetBufferType() == Graphics::BufferType::Dynamic,
						  "Only dynamic buffer type can use UpdateVertexBuffer!\n");
	...
	{
		CHERRYSODA_ASSERT(GetBufferType() == Graphics::BufferType::Dynamic,
						  "Only dynamic buffer type can use UpdateIndexbBuffer!\n");
		CHERRYSODA_ASSERT(IsValid(), "Only initialized buffer can be updated!\n");
		Graphics::UpdateDynamicIndexBuffer(m_indexBuffer, index, indices);
	}

	inline bool IsValid() const { return m_vertexBuffer != Graphics::InvalidHandle; }
