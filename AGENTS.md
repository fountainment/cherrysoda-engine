...
- High-DPI (two-axis model): three size spaces on `Engine` — `GetWindowSize` (SDL window coordinates, the mouse space: logical points on macOS, physical pixels on Windows), `GetPixelSize` (drawable pixels via `SDL_GetWindowSizeInPixels`; denser than the window on Retina because `Window::CreateWindow` sets `SDL_WINDOW_HIGH_PIXEL_DENSITY`), and `GetViewSize` (logical points for cameras, `GetRenderTargetSize(nullptr)` and engine uniforms; = window ÷ content scale). Two independent factors: content scale (`SDL_GetDisplayContentScale`, >1 only on Windows — visual compensation) and window display scale (`GetWindowDisplayScale` = pixel density × content scale — font rasterization density; 2.0 on Retina and Windows 200% alike). Swapchain/viewport always take `GetPixelSize` (`Engine::RenderCore`, `RendererBase::RenderPrepare`, `Graphics::UpdateView`); `MInput::MouseData::Position` divides window-space mouse coords by the content scale. Window events are split by space: `WINDOW_RESIZED` (logical → `Engine::OnClientSizeChanged`, the user hook) / `WINDOW_PIXEL_SIZE_CHANGED` (drawable → `bgfx::reset`) / `WINDOW_DISPLAY_SCALE_CHANGED` (refreshes both factors, re-derives the view, rebuilds GUI fonts) — cross-monitor drags and OS scale changes track at runtime with no restart. `Window::CreateWindow` also multiplies the window by the content scale on Windows (SDL takes physical pixels there; falls back to 1.0 if it would exceed the display mode), and `Window::SetSize` does the same.
- ImGui DPI (`Util/GUI.cpp`): never touch `io.DisplaySize`/`MousePos` — they share the window-coordinate space with the raw mouse feed; instead `GUI::Update` sets `DisplaySize` = window size and `DisplayFramebufferScale` = pixel density each frame (`GUI::Render` scales viewport/clip by it). Visual compensation: only when content scale > 1 (Windows), `style.FontScaleDpi` + `ScaleAllSizes` cancel the oversized DisplaySize. Rasterization: fonts bake at `GUI::FontDensity()` (= window display scale) via `cfg.RasterizerDensity` — >1 uses `AddFontDefaultVector` baked at the density, ==1 keeps `AddFontDefaultBitmap` pixel-perfect; fonts must be added after `FontScaleDpi` is set (the legacy atlas bakes at the effective size, so `FontScaleDpi` alone blur-upscales). Apps with custom fonts register a builder via `GUI::SetFontBuilder` (see ParticleEditor): it clears and repopulates `io.Fonts` at `GUI::FontDensity()` and is re-invoked automatically when the display scale changes. App-supplied pixel literals passed to ImGui layout APIs (window sizes, fixed column/item widths, image sizes) must be multiplied by `GUI::DpiScale()` (= `style.FontScaleDpi`, the same factor `ScaleAllSizes` was applied with — 1 on macOS, contentScale on Windows); `ScaleAllSizes` only covers style-internal padding/spacing, so unscaled literals render half the intended size next to the DPI-scaled fonts.
- **Run `clang-format -i` on changed C/C++ files before committing** — style lives in the root `.clang-format` (tab indent, 120 cols, Allman types/functions, attached control-flow braces, `else` on its own line). CI gates this (`.github/workflows/format.yml`, clang-format 23). Never format anything under `External/`, `out/`, `Tools/bin`, `Tools/crunch`, `Tools/sfxr_port/src/sfxr`, or `embedded_shaders/`, or the generated font blob `Tools/ParticleEditor/src/VonwaonBitmapFont.inl`. `.clang-tidy` is report-only (see its header for the run recipe); every disabled check has its reason written there.
...
bgfx-style `.sc` shaders (`vs_*.sc`/`fs_*.sc`) are compiled at build time by `Tools/compile_shader.py` into per-platform `.bin` files under `assets/shaders/`. Profiles: GLSL `430`, ESSL `300_es`, Metal, SPIR-V, and WGSL (`wgsl/` folder, for the WebGPU renderer); HLSL only compiles on Windows hosts — `s_5_0` DXBC (D3D11, `shaders/dx11/`) and `s_6_0` DXIL (D3D12, dlopens the DXC runtime sitting next to shaderc — `dxcompiler.dll`/`dxil.dll` on Windows, `libdxcompiler.so`/`libdxil.so` on Linux). Old profiles (GLSL `120`, ESSL default, DX9/`s_3_0`) were removed with the bgfx rev 9149 update; embedded DX shaders use the `_dxbc`/`_dxil` symbol suffixes bgfx's `BGFX_EMBEDDED_SHADER` macro expects. shaderc comes either as a prebuilt binary (`Tools/bin/shaderc` for Windows/Linux x64) or `Tools/bin/shaderc.local` (built automatically when configured with `CHERRYSODA_BUILD_SHADERC=ON`, e.g. the `unix-shaderc` preset — this is how you get a working shaderc on macOS). The prebuilt and the DXC runtime files beside it are refreshed together by `Tools/build_shaderc_in_docker.sh` (builds on ubuntu:22.04 for glibc reach, verifies the installed set on 24.04). Engine's own shaders live in `Engine/CherrySoda/Graphics/shaders/` and are embedded into the lib as `embedded_shaders/*.bin.h` (regenerated whenever shaderc is available).
