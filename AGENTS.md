...
- High-DPI: engine ctor width/height are logical points. `Window::CreateWindow` multiplies only the window by `SDL_GetDisplayContentScale` (Windows = OS scaling; macOS/Wayland report 1.0) and keeps the view at the logical size — viewports/swapchain work in physical window pixels (`Engine::RenderCore`, `RendererBase::RenderPrepare` for the screen pass, `Graphics::UpdateView`), while cameras, `GetRenderTargetSize(nullptr)` and engine uniforms use the logical view size, so content scales up on scaled displays. `MInput::MouseData::Position` converts physical mouse pixels to the logical view space. ImGui deliberately stays UNSCALED — 13px physical pixels, pixel-crisp, DPI applies to game content only; never touch `io.DisplaySize`/`MousePos` (mouse would offset), and if it ever needs scaling, bake the atlas at the scaled size (`cfg.SizePixels = 13 * scale` — the legacy atlas backend serves the closest baked size once locked, so `FontScaleDpi` alone just blur-upscales).
