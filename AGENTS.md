...
- High-DPI: engine ctor width/height are logical points; `Window::CreateWindow` multiplies them (and the view size, ratio-preserving) by `SDL_GetDisplayContentScale` so the window keeps its physical size under OS display scaling (Windows = OS scaling; macOS/Wayland report 1.0). View == window == mouse coords stays one physical-pixel space — keep it that way. ImGui scales only via `style.FontScaleDpi`/`ScaleAllSizes` (`Engine::GetContentScale()`), never `io.DisplaySize`/`MousePos` (mouse would offset).
