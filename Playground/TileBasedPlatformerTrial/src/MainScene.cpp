...
public:
	...
	PickUpItemCallbackComponent(STL::Action<Actor*> onPickUp) : base(false, false) { OnPickUp(std::move(onPickUp)); }

	void OnPickUp(STL::Action<Actor*> onPickUp) { m_onPickUp = std::move(onPickUp); }
...
public:
	...
	{
		if (STL::ContainsKey(m_items, id) && m_items[id] > 0) {
			--m_items[id];
			return true;
		}
	...
	void MoveY(float amount)
	{
		...
		if (move != 0) {
			...
			while (move != 0) {
				...
				if ((sign < 0) && (!CollideCheck(s_platformTag))) {
	...
	void Update() override
	{
		...
		if (frontSensor && invectory) {
			for (auto* lock : GetScene()->Get(s_lockTag)) {
				if (lock->CollideCheck(frontSensor) && invectory->RemoveItem("key")) {
					lock->RemoveSelf();
				}
			}
		}
...
public:
	...
	{
		m_followRect =
			Math::Rectangle{.m_coord = Math::Vec2(camera_follow_rect_width, camera_follow_rect_height) * -0.5f,
							.m_size = Math::Vec2(camera_follow_rect_width, camera_follow_rect_height)};
...
void MainScene::Begin()
{
	...
	for (int layerIndex = 0; std::cmp_less(layerIndex, layerArray.Size()); ++layerIndex) {
		...
		if (layer.HasMember("data") && layer["data"].IsArray()) {
			...
			for (int i = 0; std::cmp_less(i, dataArray.Size()); ++i) {
				int x = i % tilesX;
				int y = i / tilesX;
				y = tilesY - y - 1;
			...
			if (layer["name"].GetString() == String("collision")) {
				auto* mapGrid = new Grid(tilesX * 2, tilesY * 2, tileWidth / 2.f, tileHeight / 2.f);
				...
				auto* spikeGrid = new Grid(tilesX, tilesY * 2, tileWidth, tileHeight / 2.f);
	...
	player->Add(new CameraFollow(
		...
		Math::Rectangle{.m_coord = Math::Vec2(camera_follow_rect_width, camera_follow_rect_height) * -0.5f,
						.m_size = Math::Vec2(camera_follow_rect_width, camera_follow_rect_height)},
		Math::Rectangle{.m_coord = Vec2_Zero, .m_size = Math::Vec2(tilesX * tileWidth, tilesY * tileHeight)}));
