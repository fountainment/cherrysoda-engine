...
void ChunkGraphicsComponent::RebuildMesh()
{
	...
	static Color blockTypeColorMap[(int)Block::Type::Count] = {Color::White, Color::White, Color::Black,
...
void ChunkGraphicsComponent::AddQuad(const Math::Vec3& pos, float size, const Color& color, const Math::Vec3& normal)
{
	...
	if (positive) {
		GetMesh()->AddQuad(MK_VERT(pos + pVecV, cU32, normal), MK_VERT(pos, cU32, normal),
						   MK_VERT(pos + pVec, cU32, normal), MK_VERT(pos + pVecH, cU32, normal));
	}
	else {
		GetMesh()->AddQuad(MK_VERT(pos + pVec, cU32, normal), MK_VERT(pos + pVecH, cU32, normal),
						   MK_VERT(pos + pVecV, cU32, normal), MK_VERT(pos, cU32, normal));
	}
}
